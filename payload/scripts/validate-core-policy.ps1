...
$requiredMarkers = @(
    ...
    'AUDIT -> CHECKPOINT -> REMEDIATION',
    'Logical task ID',
    'continue/resume',
    'repair<=2|review<=2|meta-review<=1|proposal<=1|memory-transaction<=1|subagent-starts<=3|full-matrix<=2',
    '80%-<90%',
    '>=90%',
    'tóm tắt hiện trạng/next action',
    'chỉ resume cùng ID/counter khi dưới 90',
    'UNKNOWN` sau checkpoint phải giữ nguyên',
    'matrix 1 FAIL -> repair mới -> focused PASS',
    'focused check',
    'full matrix',
    'canonical source',
    'PASS|FAIL',
    'fallback/downgrade/switch model',
    'NO_DURABLE_LEARNING',
    'NO_MEMORY_DELTA',
    'internal phase',
    'fingerprint',
    ...
    'Agent Zero chính là orchestrator',
    '.agent/CAPABILITIES.md',
    '.agent/SUBAGENTS.md',
    '.agent/subagent-candidates/',
    '.codex/agents/',
    'registry linkage',
    'INHERIT_PARENT_RUN',
    'Catalog rỗng/không liên quan',
    'MISSING',
    'Persistent custom agent',
    'Legacy/unregistered repo skill',
    'sub-agent',
    'write ownership',
    'integration check',
    'repair count',
...
$forbiddenPatterns = @(
    ...
    @{ Pattern = '(?i)external (skill|capability|provider).{0,60}(may|can|is allowed to).{0,40}(modify|overwrite|disable|rename)'; Code = "AZ-CORE-CAPABILITY-OWNERSHIP"; Reason = "external capabilities must remain read-only" },
    @{ Pattern = '(?i)(skill|provider|custom agent).{0,80}(may|can|is allowed to).{0,40}(reset|own).{0,40}(repair|review|budget|lifecycle)'; Code = "AZ-CORE-CAPABILITY-LOOP"; Reason = "providers must inherit the parent lifecycle and budget" },
    @{ Pattern = '(?i)(high-quota authorization|USER_EXPLICIT.{0,80}(quota|usage|90)|(?:quota|usage).{0,80}(authorization bypass|bypass authorization))'; Code = "AZ-CORE-QUOTA-BYPASS"; Reason = "usage at or above 90 must checkpoint without an authorization bypass" },
    @{ Pattern = '(?i)(quota|usage).{0,80}(may|can|automatically|is allowed to).{0,60}(switch|downgrade|fallback).{0,40}(model|reasoning)'; Code = "AZ-CORE-MODEL-SWITCH"; Reason = "quota pressure must not authorize an automatic model or reasoning switch" }
)
