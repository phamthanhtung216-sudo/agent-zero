param(
    [ValidateSet("Auto", "NewProject", "ExistingProject")]
    [string]$Mode = "Auto"
)
...
$corePath = if ($Mode -eq "NewProject") { $newAgentsPath } else { $candidatePath }
if (Test-Path -LiteralPath $corePath -PathType Leaf) {
    ...
    if ($subAgentSectionMatches.Count -ne 1) {
        ...
        foreach ($marker in @("runtime", "tool trace", "definition of done", "ba sub-agent", "approval gate", "integration check", "writer", "repair count")) {
...
if ($Mode -eq "NewProject") {
    ...
    foreach ($relativeDirectory in @(".agent/lessons", ".agent/decisions", ".agent/archive/lessons", ".agent/archive/decisions", ".agent/archive/changelog")) {
    ...
    if (Test-Path -LiteralPath $adoptionPath -PathType Leaf) {
        ...
        if (-not $adoptionText.Contains('- Status: `DETECTED`')) {
...
if ($Mode -eq "ExistingProject" -and (Test-Path -LiteralPath $adoptionValidatorPath -PathType Leaf)) {
